...
import { bindConfirmDialog, type ConfirmDialogPayload, type ConfirmDialogVariant } from '../../utils/confirmDialogStore'

const variantConfig = {
  question: {
    icon: HelpCircle,
    iconBox: 'bg-sky-50 text-sky-600 ring-sky-100',
    confirm: 'bg-sky-600 text-white shadow-sm shadow-sky-100 hover:bg-sky-700 hover:shadow-md hover:shadow-sky-100',
    badge: 'text-sky-700'
  },
  danger: {
    icon: XCircle,
    iconBox: 'bg-red-50 text-red-500 ring-red-100',
    confirm: 'bg-red-500 text-white shadow-sm shadow-red-100 hover:bg-red-600 hover:shadow-md hover:shadow-red-100',
    badge: 'text-red-600'
  },
  warning: {
    icon: AlertTriangle,
...
const getVariantByConfirmText = (confirmText?: string): ConfirmDialogVariant => {
  const text = confirmText?.toLowerCase() || ''

  if (text.includes('hủy') || text.includes('xóa') || text.includes('từ chối') || text.includes('hoàn tiền')) {
    return 'danger'
  }
...
const ConfirmDialogProvider = ({ children }: { children: ReactNode }) => {
  ...
  const openConfirm = useCallback((payload: ConfirmDialogPayload) => {
  ...
  const variant = dialog?.variant || getVariantByConfirmText(dialog?.confirmText)
          ...
          >
              ...
              <div className='flex items-start justify-between gap-5'>
                <div className='flex min-w-0 items-start gap-4'>
                  ...
                  <div className='min-w-0 pt-0.5'>
                    <p className={`text-sm font-bold ${config.badge}`}>Xác nhận thao tác</p>

                    <h3 className='mt-1.5 text-2xl font-extrabold leading-snug text-slate-900'>{dialog.title}</h3>
                  </div>
                </div>
                ...
                <button
                  type='button'
                  onClick={() => closeDialog(false)}
                  className='grid h-10 w-10 shrink-0 cursor-pointer place-items-center rounded-2xl text-slate-400 transition hover:bg-slate-100 hover:text-slate-700'
                >
                  <X className='h-5 w-5' />
                </button>
              </div>
            ...
            {dialog.text && (
              <div className='px-7 py-5'>
                <p className='text-[18px] leading-7 text-slate-500'>{dialog.text}</p>
              </div>
            )}
            ...
            <div className='flex flex-col-reverse gap-2 border-t border-slate-100 bg-slate-50/70 px-7 py-5 sm:flex-row sm:justify-end'>
              <button
                ...
                onClick={() => closeDialog(false)}
                className='
                  inline-flex h-12 cursor-pointer items-center justify-center rounded-2xl
              ...
              <button
                ...
                className={`
                  inline-flex h-12 cursor-pointer items-center justify-center gap-2 rounded-2xl
